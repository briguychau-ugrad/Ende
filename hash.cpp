...
const unsigned long long Hash::VALS_SHA_512[80] = {0x428a2f98d728ae22, 0x7137449123ef65cd, 0xb5c0fbcfec4d3b2f, 0xe9b5dba58189dbbc, 0x3956c25bf348b538, 
...
char Hash::get_two(long long index, char* first, char* second, long long firstsize) {
	if (index < firstsize)
		return first[index];
	else
...
unsigned long long Hash::rotr64u(unsigned long long val, int n) {
	int ls = 64 - n;
	int rs = n;
...
char* Hash::generateSHA_256(char* input, long long size) {
	...
	for (long long k = 0; k < iterations; k++) {
		...
		for (int i = 0; i < 16; i++) {
			...
			for (int j = 0; j < 4; j++) {
				w[i] <<= 8;
				w[i] += (unsigned int)(get_two((k << 6) + (i << 2) + j, input, arr, size) & 0x000000ff);
			}
		}
		for (int i = 16; i < 64; i++) {
...
char* Hash::generateSHA_512(char* input, long long size) {
	...
	for (long long k = 0; k < iterations; k++) {
		...
		for (int i = 0; i < 16; i++) {
			...
			for (int j = 0; j < 8; j++) {
				w[i] <<= 8;
				w[i] += (unsigned long long)(get_two((k << 7) + (i << 3) + j, input, arr, size) & 0x00000000000000ffull);
			}
		}
		for (int i = 16; i < 80; i++) {
